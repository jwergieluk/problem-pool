...
\paragraph{Laplace-Verteilung. Insuffizienz des Mittelwerts.}
Consider a family $\mathcal P_{\theta\in\R}$ of double exponential distributions with densities
\begin{equation}
	p_\theta(x) = \frac{1}{2} \exp \left( -| x - \theta | \right)
\end{equation}
where $x\in\R$ and $\theta\in\R$. Suppose that sample mean $\bar X$ is sufficient for $\theta$.
Then the factorisation theorem implies there exist functions $g$ and $h$ such that
\begin{equation}
	\frac{1}{2^n} \prod_{i=1}^n \exp \left( -| x_i - \theta | \right) = g(\bar x, \theta ) h(x)
\end{equation}
for $x=(x_1,\ldots,x_n)$. Since $\exp(\cdot)$ is strictly positive we can form the quotient
\begin{equation}
	\exp\left( - \sum_{i}^{} |x_i - \theta_1| + \sum_{i}^{} |x_i -\theta_2| \right) = 
	\frac{ g(\bar x, \theta_1 ) h(x) }{g(\bar x, \theta_2 ) h(x) }.
\end{equation}
By setting 
\begin{equation}
	G(x,\theta_1, \theta_2) 
		=\log \left( \frac{g(\frac{1}{n} \sum_{i}^{} x_i,\theta_1)}{g( \frac{1}{n} \sum_{i}^{} x_i, \theta_2)} \right)
\end{equation}
we get
\begin{equation}
	- \sum_{i}^{} |x_i - \theta_1| + \sum_{i}^{} |x_i -\theta_2| = G(\sum_{i}^{} x_i ,\theta_1, \theta_2)
\end{equation}
Consider $n=2$ and the subset of $\R^2$ with $x_1+x_2=0$. Setting $\theta_1=0$ and $\theta_2=1$ yields
\begin{equation}
	-|x_1|-|x_2| + |x_1-1| + |x_2-1| = G(x_1+x_2, 0,1)= G(0, 0,1)
	\label{punch-line}
\end{equation}
This means $G(0,0,1)$ is constant on $x_1+x_2=0$ but the left hand side of
(\ref{punch-line}) is not. With $x_2=-x_1$ we get 
\begin{equation}
	-|x_1|-|-x_1| + |x_1-1| + |-x_1-1| = -2|x_1| + |x_1-1| + |x_1+1|
\end{equation}
which is
\href{http://www.wolframalpha.com/input/?i=plot+%28-%7Cx_1%7C-%7C-x_1%7C+%2B+%7Cx_1-1%7C+%2B+%7C-x_1-1%7C%29}{not
a constant function} of $x_1$. A contradiction. 
...
stetig gleichverteilt auf dem Intervall $[0,\theta]$ mit $\theta>0$. 
...
\end{align*}
Insbesondere ist der auf dem ersten Moment basierende Schätzer
\begin{equation*}
