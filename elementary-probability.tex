...
verteilt mit dem Parameter $p$. Zeigen Sie, dass 
\begin{equation*}
    P \left( X > i+j \,|\, X>i \right) = P\left( X > j \right) 
\end{equation*}
für alle $i,j>0$ gilt.
...
\begin{enumerate}
    \item Für das Maximum der Wahrscheinlichkeitsfunktion von $X$ gilt
        \begin{equation*}
            \max_{k\in \left\{ 0,\dots ,n \right\}} P(X=k) = \lfloor (n+1)p \rfloor.
        \end{equation*}
        Dabei bezeichnet $\lfloor x \rfloor$ die größte ganze Zahl kleiner als $x$. 

    \item $X$ is gerade mit Wahrscheinlichkeit
        \begin{equation*}
            P\left( X \text{ ist gerade} \right) = \frac{1}{2}\left( 1 + (1-2p)^n \right). 
        \end{equation*}
\end{enumerate}
...
\begin{enumerate}
    \item Für das Maximum der Wahrscheinlichkeitsfunktion von $X$ gilt
        \begin{equation*}
            \max_{k\in \bN} P(X= k) = \lfloor \lambda \rfloor. 
        \end{equation*}

    \item $\bfE | X -\lambda | = \frac{2 \lambda^{\lambda} e^{-\lambda}}{ (\lambda-1)!}$.
...
\begin{equation*}
    Y = X_1 + X_2
\end{equation*}
ebenfalls poissonverteilt mit dem Parameter $\lambda=\lambda_1 + \lambda_2$ ist. 
...
und daraus
\begin{equation*}
    1 - B(k+1, n, p) = n \binom{n-1}{k} \int_{0}^{p} t^{k} (1-t)^{n-k-1} dt.
\end{equation*}
