...
\paragraph*{Lösung.} $\bfE X = \frac{n+1}{2}$, $\sum_{k=1}^{n} k^2 = \frac{n(n+1)(2n+1)}{6}$,
...
$X$ und $Y$ sind unabhängig und geometrisch verteilt mit dem Parameter $p\in(0,1)$.
\begin{enumerate}
    ...
    \item Zeigen Sie, dass für $k,n\in\bN$ und $n>k$
        ...
        \end{equation*}
        gilt. Mit anderen Worten, die bedingte Verteilung von $X$ gegeben
...
\end{equation*}
gilt. Seien $Z_1,Z_2\dots$ unabhängig geometrisch verteilte Zufallsgrößen mit 
...
$X_1,\dots ,X_m$ seien unabhängig und diskret gleichverteilt auf der Menge
...
\begin{enumerate}
    \item Finden Sie die Wahrscheinlichkeitsfunktion der Zufallsgrößen
        $X_{(1)}$ und $X_{(n)}$.
...
unabhängig und geometrisch verteilt mit dem Parameter $p\in(0,1)$.  Finden Sie
