...
\section{Einfache Zufallsvariablen}


\paragraph{Einfache Zufallsvariablen. Bildmaß.}
Seien $\left( \Omega, \fA, P \right)$ ein Wahrscheinlichkeitsmaß und eine
Zufallsgröße $X: \Omega \to \bR$ gegeben, wobei die Menge der reellen Zahlen
gemeinsam mit der Borel'schen $\sigma$-Algebra einen Meßraum $\left( \bR,
\cB(\bR) \right)$ bildet. Zeigen Sie folgende Aussagen:
\begin{enumerate}
    ...
    \item Ist die Ergebnismenge $\Omega = \left\{ \omega_{1},\dots ,\omega_{n} \right\}$ endlich,
        ...
        \end{equation*}
        Dabei ist $\delta_x : \cB(\cR) \to \R$ für ein festes $x\in\bR$ ein
        Diracmaß mit $\delta_x(\left\{ x \right\})=1$. Sind die Tupel
...
\begin{enumerate}
    ...
    \item Berechnen Sie den $k$-ten Moment und den $k$-ten zentralen Moment von 
        $X$ für alle $k\in \bN$. 
\end{enumerate}


\paragraph{Zufallsgrößen. Diskrete Gleichverteilung.} Eine Zufallsgröße
heißt gleichverteilt auf $\left\{ 1,\dots ,n \right\}$ falls $P(X=i)=\frac{1}{n}$
für alle $i\in\left\{ 1,\dots ,n \right\}$.
\begin{enumerate}
    \item Geben Sie die Verteilungsfunktion $F: \bR\to [0,1], x\mapsto F(x) =
        P(X<x)$ von $X$ an.
...
\begin{enumerate}
    \item $\bfE \left( X^2 - \bfE X \right)^2 = \bfE X^2 - \left( \bfE X \right)^2$, falls alle diese
