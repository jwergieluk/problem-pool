...
folgende Aussagen:
\begin{enumerate}
    \item Angenommen $A\cap B=\emptyset$. $A$ und $B$ sind genau dann
        unabhängig, wenn $P(A)=0$ oder $P(B)=0$ gilt.

    \item Sind $A$ und $B$ unabhängig und $A \cup B = \Omega$, dann gilt 
        $P(A)=1$ oder $P(B)=1$. 
\end{enumerate}


\paragraph{Unabhängigkeit. Gleichwahrscheinliche Ereignisse.} Seien $(\Omega,
\fA, P)$ ein Wahrscheinlichkeitsraum und $A_1,\cdots, A_n \in\fA, n\in\bN$
vollständig stochastisch unabhängige Ereignisse mit $P(A_i)=p\in[0,1]\ \forall
i\in \left\{ 1,\cdots,n \right\}$ gegeben. Berechnen Sie die
Wahrscheinlichkeiten der folgenden Ereignisse:
\begin{enumerate}
    \item Alle Ereignisse $A_1,\cdots, A_n$ treten ein.
    \item Keins der Ereignisse $A_1, \cdots, A_n$ tritt ein. 
    \item Genau eins der Ereignisse $A_1, \cdots, A_n$ tritt ein. Wieso 
...
Bezeichnen wir als $K_i, i\in \left\{1,\cdots,n\right\}$ das Ereignis, dass
beim $i$-ten Wurf Kopf fällt. 
\begin{enumerate}
    \item Finden Sie einen geeigneten Wahrscheinlichkeitsraum, der diese
        Zufallssituation beschreibt.
    \item Zeigen Sie, dass die Ereignisse $K_1,\cdots, K_n$ vollständig 
...
$(\Omega, \fA, P)$ ein Wahrscheinlichkeitsraum und $A_1, \cdots, A_n \in\fA$
vollständig stochastisch unabhängige Ereignisse  gegeben. Zeigen Sie:
\begin{equation*}
    P( \overline{A_1 \cup \cdots \cup A_n} ) \leq \exp \left( - \sum_{i=1}^{n} P(A_i) \right).
\end{equation*}
