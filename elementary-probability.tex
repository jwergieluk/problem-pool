...
\paragraph{Exponentialverteilung. Dichte vom Maximum.} Seien $X_1, \dots, X_n$
stochastisch unabhängig und exponentialverteilt mit $X_i\sim
\mathbf{Ex}(\lambda)$ und $\lambda>0$. Berechnen Sie die Dichte der
...
\paragraph{Aktie im Bachelier-Modell.} Die Kursveränderungen einer Aktie eines
...
\begin{enumerate}
    \item Geben Sie je eine erwartungstreue Schätzfunktion für den Mittelwert und für
        die Varianz der Kursveränderungen an. Berechnen Sie diese Schätzwerte. 
    \item Ein Analyst behauptet $\mu>0$ und gibt eine Kaufempfehlung aus. Lässt
        ...
        Irrtumswahrscheinlichkeit von $5\%$ rechtfertigen?
\end{enumerate}

\paragraph{Übertragungskanal.} Die Fehlerrate eines Übertragungskanals hängt
von der Entfernung $D$ (im m) zwischen Quelle und Senke. Die
Wahrscheinlichkeit, dass ein Bit richtig übertragen wird, beträgt $p =
0.001\cdot{} D$ für $D\leq 500\,\text{m}$ und $p=0.5$ für $D>500\,\text{m}$.
Das heißt, bei Entfernungen von $500\,\text{m}$ oder größer wird nur noch
Rauschen an der Senke empfangen.

Es wird eine $100$ Bit Nachricht zur Übertragung vorbereitet.  
\begin{enumerate}
    \item Die Entfernung sei $D=100\,$m. Berechnen Sie die Wahrscheinlichkeit dafür,
        dass mehr als $20$ Bits von der obigen Nachricht falsch übertragen
        werden. 

    \item Zur Verminderung der Fehlerrate wird jedes Bit der Nachricht $8$ mal
        hintereinander übertragen. Ein Decodieralgorithmus an der Senke zerlegt
        die Nachricht in $8$ Bit Teile und gibt für jeden Teil $1$ aus, falls
        $4$ oder mehr Bit gleich $1$ sind und sonst $0$. Berechnen Sie bei
        $D=100\,m$ die Wahrscheinlichkeit dafür, dass mehr als $20$ Bits von
        der ursprünglichen $100$ Bit langen Nachricht falsch übertragen werden.
...
\paragraph{GPS.} Nachdem die Verbindung zu $k$ Satelliten aufgebaut wurde
...
\begin{enumerate}
    ...
    \item Wir testen
        \begin{align*}
            H_0: \mu < 5 & \text{ gegen } & H_1: \mu=5.
        \end{align*}
        und verwenden die Teststatistik
        \begin{equation*}
            T = \frac{\bar X_n - 5}{S_n} \sqrt{n} \sim t_{n-1}.
        \end{equation*}
        Als kritischer Bereich ergibt sich $K = ( t_{n-1, 1-\alpha}, +\infty)$.
\end{enumerate}
